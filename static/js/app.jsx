const { useState, useEffect, useRef, useCallback, useContext, createContext } = React;
    ...
    const DetailView = () => {
      ...
      useEffect(() => {
        ...
        if (isFirstRefreshRun.current) { isFirstRefreshRun.current = false; return; }
    ...
    const App = () => {
      const [currentView, setView] = useState(() => {
        ...
        return view ? { name: view } : { name: 'home' };
      });
      const [detailContext, setDetailContext] = useState(() => {
        const params = new URLSearchParams(window.location.search);
        if (params.get('view') === 'detail' && params.get('type') && params.get('id')) {
           return { type: params.get('type'), data: { id: parseInt(params.get('id')), title: 'Loading...', songs: [] } };
        }
        return null;
      });
      ...
      useEffect(() => {
        const handlePopState = () => {
          ...
          if (view) {
            setView({ name: view });
            if (view === 'detail') {
               setDetailContext({ type: params.get('type'), data: { id: parseInt(params.get('id')), title: 'Loading...', songs: [] } });
            }
          } else {
            setView({ name: 'home' });
          }
